...
Sessions help the server manage the authentication of users
...
bcrypt uses a salt to hash passwords requiring a brute force attacker to create a rainbow table for each user record.it also runs the hashing a number of times. Also, the bcrypt algorithm is made to be a bit slow and so this would slow down attackers using brute force.
